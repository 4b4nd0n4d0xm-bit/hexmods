--[[
	YIMF0RCEDEV Interface Suite
	by Sirius (Modified for YIMF0RCEDEV)
...
local requestsDisabled = true --getgenv and getgenv().DISABLE_YIMF0RCEDEV_REQUESTS
local InterfaceBuild = '3K3W'
local Release = "Build 1.69"
local YIMF0RCEDEVFolder = "YIMF0RCEDEV"
local ConfigurationFolder = YIMF0RCEDEVFolder.."/Configurations"
local ConfigurationExtension = ".yfld"
local settingsTable = {
	General = {
		-- if needs be in order just make getSetting(name)
		yimforceOpen = {Type = 'bind', Value = 'K', Name = 'YIMF0RCEDEV Keybind'},
		-- buildwarnings
		-- yimforceprompts
...
local overriddenSettings: { [string]: any } = {} -- For example, overriddenSettings["System.yimforceOpen"] = "J"
...
local function callSafely(func, ...)
	if func then
		...
		if not success then
			warn("YIMF0RCEDEV | Function failed with error: ", result)
			return false
		else
			return result
		end
...
local function loadSettings()
	...
	local success, result =	pcall(function()
		if callSafely(isfolder, YIMF0RCEDEVFolder) then
			if callSafely(isfile, YIMF0RCEDEVFolder..'/settings'..ConfigurationExtension) then
				file = callSafely(readfile, YIMF0RCEDEVFolder..'/settings'..ConfigurationExtension)
			end
		end
		...
		if useStudio then
			file = [[
	{"General":{"yimforceOpen":{"Value":"K","Type":"bind","Name":"YIMF0RCEDEV Keybind","Element":{"HoldToInteract":false,"Ext":true,"Name":"YIMF0RCEDEV Keybind","Set":null,"CallOnChange":true,"Callback":null,"CurrentKeybind":"K"}}},"System":{"usageAnalytics":{"Value":false,"Type":"toggle","Name":"Anonymised Analytics","Element":{"Ext":true,"Name":"Anonymised Analytics","Set":null,"CurrentValue":false,"Callback":null}}}}
]]
		end
		...
		-- If no settings saved, apply overridden settings only
			...
			for settingName, settingValue in overriddenSettings do
				...
				assert(#split == 2, "YIMF0RCEDEV | Invalid overridden setting name: " .. settingName)
	...
	if not success then 
		if writefile then
			warn('YIMF0RCEDEV had an issue accessing configuration saving capability.')
		end
	end
end
...
if not requestsDisabled then
	...
	if cachedSettings and (#cachedSettings == 0 or (cachedSettings.System and cachedSettings.System.usageAnalytics and cachedSettings.System.usageAnalytics.Value)) then
		sendReport("execution", "YIMF0RCEDEV")
...
local YIMF0RCEDEV = useStudio and script.Parent:FindFirstChild('YIMF0RCEDEV') or game:GetObjects("rbxassetid://10804731440")[1]
YIMF0RCEDEV.Name = "YIMF0RCEDEV"
local buildAttempts = 0
local correctBuild = false
local warned
local globalLoaded
local yimforceDestroyed = false -- True when YIMF0RCEDEVLibrary:Destroy() is called

repeat
	if YIMF0RCEDEV:FindFirstChild('Build') and YIMF0RCEDEV.Build.Value == InterfaceBuild then
		correctBuild = true
		break
	end
	...
	if not warned then
		warn('YIMF0RCEDEV | Build Mismatch')
		print('YIMF0RCEDEV may encounter issues as you are running an incompatible interface version ('.. ((YIMF0RCEDEV:FindFirstChild('Build') and YIMF0RCEDEV.Build.Value) or 'No Build') ..').\n\nThis version of YIMF0RCEDEV is intended for interface build '..InterfaceBuild..'.')
		warned = true
	end

	local toDestroy
	toDestroy, YIMF0RCEDEV = YIMF0RCEDEV, useStudio and script.Parent:FindFirstChild('YIMF0RCEDEV') or game:GetObjects("rbxassetid://10804731440")[1]
	if toDestroy and not useStudio then toDestroy:Destroy() end

	buildAttempts = buildAttempts + 1
until buildAttempts >= 2

YIMF0RCEDEV.Enabled = false

if gethui then
	YIMF0RCEDEV.Parent = gethui()
elseif syn and syn.protect_gui then 
	syn.protect_gui(YIMF0RCEDEV)
	YIMF0RCEDEV.Parent = CoreGui
elseif not useStudio and CoreGui:FindFirstChild("RobloxGui") then
	YIMF0RCEDEV.Parent = CoreGui:FindFirstChild("RobloxGui")
...
if gethui then
	for _, Interface in ipairs(gethui():GetChildren()) do
		if Interface.Name == YIMF0RCEDEV.Name and Interface ~= YIMF0RCEDEV then
			Interface.Enabled = false
			Interface.Name = "YIMF0RCEDEV-Old"
		end
	end
elseif not useStudio then
...
if YIMF0RCEDEV.AbsoluteSize.X < minSize.X and YIMF0RCEDEV.AbsoluteSize.Y < minSize.Y then
...
local Main = YIMF0RCEDEV.Main
local MPrompt = YIMF0RCEDEV:FindFirstChild('Prompt')
local Topbar = Main.Topbar
local Elements = Main.Elements
local LoadingFrame = Main.LoadingFrame
local TabList = Main.TabList
local dragBar = YIMF0RCEDEV:FindFirstChild('Drag')
...
YIMF0RCEDEV.DisplayOrder = 100
LoadingFrame.Version.Text = Release

-- Thanks to Latte Softworks for the Lucide integration for Roblox
local Icons = useStudio and require(script.Parent.icons) or loadWithTimeout('https://raw.githubusercontent.com/SiriusSoftwareLtd/YIMF0RCEDEV/refs/heads/main/icons.lua')
-- Variables
...
local Notifications = YIMF0RCEDEV.Notifications
local keybindConnections = {} -- For storing keybind connections to disconnect when YIMF0RCEDEV is destroyed

local SelectedTheme = YIMF0RCEDEVLibrary.Theme.Default

local function ChangeTheme(Theme)
	if typeof(Theme) == 'string' then
		SelectedTheme = YIMF0RCEDEVLibrary.Theme[Theme]
	elseif typeof(Theme) == 'table' then
		SelectedTheme = Theme
	end

	YIMF0RCEDEV.Main.BackgroundColor3 = SelectedTheme.Background
	YIMF0RCEDEV.Main.Topbar.BackgroundColor3 = SelectedTheme.Topbar
	YIMF0RCEDEV.Main.Topbar.CornerRepair.BackgroundColor3 = SelectedTheme.Topbar
	YIMF0RCEDEV.Main.Shadow.Image.ImageColor3 = SelectedTheme.Shadow

	YIMF0RCEDEV.Main.Topbar.ChangeSize.ImageColor3 = SelectedTheme.TextColor
	YIMF0RCEDEV.Main.Topbar.Hide.ImageColor3 = SelectedTheme.TextColor
	YIMF0RCEDEV.Main.Topbar.Search.ImageColor3 = SelectedTheme.TextColor
	if Topbar:FindFirstChild('Settings') then
		YIMF0RCEDEV.Main.Topbar.Settings.ImageColor3 = SelectedTheme.TextColor
		YIMF0RCEDEV.Main.Topbar.Divider.BackgroundColor3 = SelectedTheme.ElementStroke
	end
	...
	for _, text in ipairs(YIMF0RCEDEV:GetDescendants()) do
...
local function getAssetUri(id: any): string
	...
	elseif type(id) == "string" and not Icons then
		warn("YIMF0RCEDEV | Cannot use Lucide icons as icons library is not loaded")
	else
		warn("YIMF0RCEDEV | The icon argument must either be an icon ID (number) or a Lucide icon name (string)")
	end
	return assetUri
end
...
local function LoadConfiguration(Configuration)
	...
	if not success then warn('YIMF0RCEDEV had an issue decoding the configuration file, please try delete the file and reopen YIMF0RCEDEV.') return end

	-- Iterate through current UI elements' flags
	for FlagName, Flag in pairs(YIMF0RCEDEVLibrary.Flags) do
		...
		if (typeof(FlagValue) == 'boolean' and FlagValue == false) or FlagValue then
			...
			warn("YIMF0RCEDEV | Unable to find '"..FlagName.. "' in the save file.")
			print("The error above may not be an issue if new elements have been added or not been set values.")
			--YIMF0RCEDEVLibrary:Notify({Title = "YIMF0RCEDEV Flags", Content = "YIMF0RCEDEV was unable to find '"..FlagName.. "' in the save file. Check sirius.menu/discord for help.", Image = 3944688398})
		end
	end
...
local function SaveConfiguration()
	...
	local Data = {}
	for i, v in pairs(YIMF0RCEDEVLibrary.Flags) do
...
function YIMF0RCEDEVLibrary:Notify(data) -- action e.g open messages
	task.spawn(function()
		...
		if data.Actions then
			warn('YIMF0RCEDEV | Not seeing your actions in notifications?')
...
local function Hide(notify: boolean?)
	...
	if notify then
		if useMobilePrompt then 
			YIMF0RCEDEVLibrary:Notify({Title = "Interface Hidden", Content = "The interface has been hidden, you can unhide the interface by tapping 'Show'.", Duration = 7, Image = 4400697855})
		else
			YIMF0RCEDEVLibrary:Notify({Title = "Interface Hidden", Content = "The interface has been hidden, you can unhide the interface by tapping " .. tostring(getSetting("General", "yimforceOpen")) .. ".", Duration = 7, Image = 4400697855})
		end
	end
...
local function saveSettings() -- Save settings to config file
	...
	if success then
		...
		callSafely(writefile, YIMF0RCEDEVFolder..'/settings'..ConfigurationExtension, encoded)
	end
end
...
local function createSettings(window)
	...
	local newTab = window:CreateTab('YIMF0RCEDEV Settings', 0, true)

	if TabList['YIMF0RCEDEV Settings'] then
		TabList['YIMF0RCEDEV Settings'].LayoutOrder = 1000
	end

	if Elements['YIMF0RCEDEV Settings'] then
		Elements['YIMF0RCEDEV Settings'].LayoutOrder = 1000
	end
...
function YIMF0RCEDEVLibrary:CreateWindow(Settings)
	if YIMF0RCEDEV:FindFirstChild('Loading') then
		if getgenv and not getgenv().yimforceCached then
			YIMF0RCEDEV.Enabled = true
			YIMF0RCEDEV.Loading.Visible = true

			task.wait(1.4)
			YIMF0RCEDEV.Loading.Visible = false
		end
	end

	if getgenv then getgenv().yimforceCached = true end

	if not correctBuild and not Settings.DisableBuildWarnings then
		task.delay(3, 
			function() 
				YIMF0RCEDEVLibrary:Notify({Title = 'Build Mismatch', Content = 'YIMF0RCEDEV may encounter issues as you are running an incompatible interface version ('.. ((YIMF0RCEDEV:FindFirstChild('Build') and YIMF0RCEDEV.Build.Value) or 'No Build') ..').\n\nThis version of YIMF0RCEDEV is intended for interface build '..InterfaceBuild..'.\n\nTry rejoining and then run the script twice.', Image = 4335487866, Duration = 15})		
			end)
	end
	...
	if not Settings.DisableYIMF0RCEDEVPrompts then
		task.spawn(function()
			while true do
	...
	if Settings.Discord and Settings.Discord.Enabled and not useStudio then
		ensureFolder(YIMF0RCEDEVFolder.."/Discord Invites")

		if callSafely(isfile, YIMF0RCEDEVFolder.."/Discord Invites".."/"..Settings.Discord.Invite..ConfigurationExtension) then
			if requestFunc then
				pcall(function()
					requestFunc({
			...
			if Settings.Discord.RememberJoins then -- We do logic this way so if the developer changes this setting, the user still won't be prompted, only new users
				callSafely(writefile, YIMF0RCEDEVFolder.."/Discord Invites".."/"..Settings.Discord.Invite..ConfigurationExtension,"YIMF0RCEDEV RememberJoins is true for this invite, this invite will not ask you to join again")
			end
		end
	end
	...
	if (Settings.KeySystem) then
		...
		ensureFolder(YIMF0RCEDEVFolder.."/Key System")
		...
		if Settings.KeySettings.GrabKeyFromSite then
			for i, Key in ipairs(Settings.KeySettings.Key) do
				...
				if not Success then
					print("YIMF0RCEDEV | "..Key.." Error " ..tostring(Response))
					warn('Check docs.sirius.menu for help with YIMF0RCEDEV specific development.')
				end
			end
		end
		...
		if callSafely(isfile, YIMF0RCEDEVFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension) then
			for _, MKey in ipairs(Settings.KeySettings.Key) do
				local savedKeys = callSafely(readfile, YIMF0RCEDEVFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension)
			...
			KeyUI.Main.Input.InputBox.FocusLost:Connect(function()
				...
				if KeyFound then
					...
					if Settings.KeySettings.SaveKey then
						callSafely(writefile, YIMF0RCEDEVFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension, FoundKey)
						YIMF0RCEDEVLibrary:Notify({Title = "Key System", Content = "The key for this script has been saved successfully.", Image = 3605522284})
					end
				else
	...
	function Window:CreateTab(Name, Image, Ext)
		...
		function Tab:CreateButton(ButtonSettings)
			...
			Button.Interact.MouseButton1Click:Connect(function()
				...
				-- Prevents animation from trying to play if the button's callback called YIMF0RCEDEVLibrary:Destroy()
				if yimforceDestroyed then
					return
				end
				if not Success then
					...
					print("YIMF0RCEDEV | "..ButtonSettings.Name.." Callback Error " ..tostring(Response))
					warn('Check docs.sirius.menu for help with YIMF0RCEDEV specific development.')
					task.wait(0.5)
					Button.Title.Text = ButtonSettings.Name
		...
		function Tab:CreateColorPicker(ColorPickerSettings) -- by Throit
			...
			ColorPicker.Interact.MouseButton1Down:Connect(function()
				...
				if not opened then
					...
					TweenService:Create(Main, TweenInfo.new(0.2, Enum.EasingStyle.Exponential), {ImageTransparency = SelectedTheme ~= YIMF0RCEDEVLibrary.Theme.Default and 0.25 or 0.1}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and ColorPickerSettings.Flag then
					YIMF0RCEDEVLibrary.Flags[ColorPickerSettings.Flag] = ColorPickerSettings
				end
			end
			...
			YIMF0RCEDEV.Main:GetPropertyChangedSignal('BackgroundColor3'):Connect(function()
				for _, rgbinput in ipairs(ColorPicker.RGB:GetChildren()) do
					if rgbinput:IsA("Frame") then
						rgbinput.BackgroundColor3 = SelectedTheme.InputBackground
						rgbinput.UIStroke.Color = SelectedTheme.InputStroke
		...
		function Tab:CreateInput(InputSettings)
			...
			Input.InputFrame.InputBox.FocusLost:Connect(function()
				...
				if not Success then
					...
					print("YIMF0RCEDEV | "..InputSettings.Name.." Callback Error " ..tostring(Response))
					warn('Check docs.sirius.menu for help with YIMF0RCEDEV specific development.')
					task.wait(0.5)
					Input.Title.Text = InputSettings.Name
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and InputSettings.Flag then
					YIMF0RCEDEVLibrary.Flags[InputSettings.Flag] = InputSettings
				end
			end
		...
		function Tab:CreateDropdown(DropdownSettings)
				...
				for _, Option in ipairs(DropdownSettings.Options) do
					...
					DropdownOption.Interact.MouseButton1Click:Connect(function()
						...
						if not Success then
							...
							print("YIMF0RCEDEV | "..DropdownSettings.Name.." Callback Error " ..tostring(Response))
							warn('Check docs.sirius.menu for help with YIMF0RCEDEV specific development.')
							task.wait(0.5)
							Dropdown.Title.Text = DropdownSettings.Name
							TweenService:Create(Dropdown, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and DropdownSettings.Flag then
					YIMF0RCEDEVLibrary.Flags[DropdownSettings.Flag] = DropdownSettings
				end
			end
		...
		function Tab:CreateKeybind(KeybindSettings)
				...
				elseif not KeybindSettings.CallOnChange and KeybindSettings.CurrentKeybind ~= nil and (input.KeyCode == Enum.KeyCode[KeybindSettings.CurrentKeybind] and not processed) then -- Test
					...
					if not KeybindSettings.HoldToInteract then
						...
						if not Success then
							...
							print("YIMF0RCEDEV | "..KeybindSettings.Name.." Callback Error " ..tostring(Response))
							warn('Check docs.sirius.menu for help with YIMF0RCEDEV specific development.')
							task.wait(0.5)
							Keybind.Title.Text = KeybindSettings.Name
							TweenService:Create(Keybind, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and KeybindSettings.Flag then
					YIMF0RCEDEVLibrary.Flags[KeybindSettings.Flag] = KeybindSettings
				end
			end
		...
		function Tab:CreateToggle(ToggleSettings)
			...
			if SelectedTheme ~= YIMF0RCEDEVLibrary.Theme.Default then
			...
			Toggle.Interact.MouseButton1Click:Connect(function()
				...
				if not Success then
					...
					print("YIMF0RCEDEV | "..ToggleSettings.Name.." Callback Error " ..tostring(Response))
					warn('Check docs.sirius.menu for help with YIMF0RCEDEV specific development.')
					task.wait(0.5)
					Toggle.Title.Text = ToggleSettings.Name
					TweenService:Create(Toggle, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if not ToggleSettings.Ext then
				if Settings.ConfigurationSaving then
					if Settings.ConfigurationSaving.Enabled and ToggleSettings.Flag then
						YIMF0RCEDEVLibrary.Flags[ToggleSettings.Flag] = ToggleSettings
					end
				end
			end
		...
		function Tab:CreateSlider(SliderSettings)
					...
					if SLDragging then
						...
						if SliderSettings.CurrentValue ~= NewValue then
							...
							if not Success then
								...
								print("YIMF0RCEDEV | "..SliderSettings.Name.." Callback Error " ..tostring(Response))
								warn('Check docs.sirius.menu for help with YIMF0RCEDEV specific development.')
								task.wait(0.5)
								Slider.Title.Text = SliderSettings.Name
								TweenService:Create(Slider, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and SliderSettings.Flag then
					YIMF0RCEDEVLibrary.Flags[SliderSettings.Flag] = SliderSettings
				end
			end
	...
	function Window.ModifyTheme(NewTheme)
		...
		if not success then
			YIMF0RCEDEVLibrary:Notify({Title = 'Unable to Change Theme', Content = 'We are unable find a theme on file.', Image = 4400704299})
		else
			YIMF0RCEDEVLibrary:Notify({Title = 'Theme Changed', Content = 'Successfully changed theme to '..(typeof(NewTheme) == 'string' and NewTheme or 'Custom Theme')..'.', Image = 4483362748})
		end
	end
	...
	if not success then warn('YIMF0RCEDEV had an issue creating settings.') end
...
function YIMF0RCEDEVLibrary:SetVisibility(visibility: boolean)
	setVisibility(visibility, false)
end

function YIMF0RCEDEVLibrary:IsVisible(): boolean
	return not Hidden
end

local hideHotkeyConnection -- Has to be initialized here since the connection is made later in the script
function YIMF0RCEDEVLibrary:Destroy()
	yimforceDestroyed = true
...
if Topbar:FindFirstChild('Settings') then
	Topbar.Settings.MouseButton1Click:Connect(function()
		task.spawn(function()
			...
			Elements.UIPageLayout:JumpTo(Elements['YIMF0RCEDEV Settings'])
		end)
	end)
...
hideHotkeyConnection = UserInputService.InputBegan:Connect(function(input, processed)
	if (input.KeyCode == Enum.KeyCode[getSetting("General", "yimforceOpen")]) and not processed then
		if Debounce then return end
		if Hidden then
			Hidden = false
			Unhide()
...
function YIMF0RCEDEVLibrary:LoadConfiguration()
	local config
	...
	if CEnabled then
		...
		local success, result = pcall(function()
			...
			if isfile then 
				...
				notified = true
				YIMF0RCEDEVLibrary:Notify({Title = "YIMF0RCEDEV Configurations", Content = "We couldn't enable Configuration Saving as you are not using software with filesystem support.", Image = 4384402990})
			end
		end)

		if success and loaded and not notified then
			YIMF0RCEDEVLibrary:Notify({Title = "YIMF0RCEDEV Configurations", Content = "The configuration file for this script has been loaded from a previous session.", Image = 4384403532})
		elseif not success and not notified then
			warn('YIMF0RCEDEV Configurations Error | '..tostring(result))
			YIMF0RCEDEVLibrary:Notify({Title = "YIMF0RCEDEV Configurations", Content = "We've encountered an issue loading your configuration correctly.\n\nCheck the Developer Console for more information.", Image = 4384402990})
		end
	end
...
task.delay(4, function()
	YIMF0RCEDEVLibrary.LoadConfiguration()
